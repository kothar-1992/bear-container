...
public class HideRecorder {
    ...
    public static void setFakeRecorderWindowLayoutParams(WindowManager.LayoutParams layoutParams) {
        try {
            ...
            layoutParams.setTitle(HideRecorder.getFakeRecordWindowTitle());
            /* 下面是特殊的 */
            if (check(ROM_FLYME)) {
                /* 理论全系魅族 */
                if (!setMeizuParams(layoutParams)) {
                    if (isActivice()) {
                        setMeizuParams_new(layoutParams); //最新魅族
                    }
                }
                ...
                @SuppressLint("SoonBlockedPrivateApi") Field privateflagField = layoutParams.getClass().getDeclaredField("PRIVATE_FLAG_IS_ROUNDED_CORNERS_OVERLAY");
    ...
    private static void setOnePulsParams(WindowManager.LayoutParams params, int flagValue) {
        try {
            @SuppressLint("DiscouragedPrivateApi") Field flagField = params.getClass().getDeclaredField("privateFlags");
            flagField.setAccessible(true);
